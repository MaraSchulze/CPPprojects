...
AForm::AForm() : name("default"), isSigned(false), requiredGradeToSign(150), requiredGradeToExecute(150), target("default") {}
...
void	AForm::execute(Bureaucrat const & executor) const {
	if (! (isSigned && executor.getGrade() <= getRequiredGradeToExecute()))
		throw GradeTooLowException();
	executor.getName();
	this->execute_action();
}
